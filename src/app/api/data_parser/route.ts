  ...
  for (let i = 2 * keys.length; i < data.length; i += keys.length) {
    ...
    for (let j = 0; j < keys.length; j++) {
      if (j == 0) {
        ...
        const cleanedDateString = dateString.replace('오전', '').replace('오후', '').trim().replace(/\s+/g, ' ');
        ...
        group[keys[j]] = new Date(cleanedYear, cleanedMonth - 1, cleanedDay, hour, minute, second);
      }
...
function animalizeName(data_group: DataGroup[]): DataGroup[] {
  const animalNames = [
    "Alligator", "Owl", "Giraffe", "Unicorn", "Rhino",
    "Emu", "Jaguar", "Mongoose", "Elephant", "Camel", 
    "Gazelle", "Beaver", "Raccoon", "Falcon", "Gecko", 
    "Otter", "Lion", "Fox", "Koala", "Ferret", "Penguin", 
    "Dolphin", "Aardvark", "Flamingo", "Caribou", "Kangaroo", 
    "Eagle", "Meerkat", "Hedgehog", "Bison", "Shark", 
    "Chipmunk", "Ocelot", "Dugong", "Squirrel", "Quokka", 
    "Bonobo", "Parrot", "Leopard", "Buffalo", "Llama", 
    "Coyote", "Narwhal", "Armadillo", "Cheetah", "Bluejay", 
    "Tiger", "Alpaca", "Hyena", "Bat", "Dingo", "Chameleon", 
    "Panda", "Crocodile", "Monkey", "Cockatoo", "Zebra", "Antelope", "Ibe"
  ];
}
